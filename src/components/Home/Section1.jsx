...
const HeroCarousel = () => {
    ...
    useEffect(() => {
        ...
        const fetchTrendingMovies = async () => {
            const response = await getTMDBData("trending/all/day", { language: 'en-US', page: 1 });
    ...
    const currentMovieGenres = trendingMovies[currentMovieIndex] ? getGenreNames(trendingMovies[currentMovieIndex].genre_ids) : [];

    // ============ NAVIGATION HANDLERS ============
    const goToNextMovie = () => {
        setCurrentMovieIndex(prev => trendingMovies.length ? (prev + 1) % trendingMovies.length : 0);
    };
    const goToPreviousMovie = () => {
        setCurrentMovieIndex(prev => trendingMovies.length ? (prev - 1 + trendingMovies.length) % trendingMovies.length : 0);
    };
                        ...
                        <div className="absolute inset-0 flex flex-col justify-center items-start pl-8 md:pl-16">
                            <div className="max-w-3xl space-y-6">
                                ...
                                <h1 className="text-5xl md:text-7xl font-black text-white drop-shadow-2xl leading-tight">
                                    {trendingMovies[currentMovieIndex].title || trendingMovies[currentMovieIndex].name}
                                </h1>
                                ...
                                {currentMovieGenres.length > 0 && (
                                    <div className="flex flex-wrap gap-2">
                                        {currentMovieGenres.map((genre, idx) => (
                                            <span key={idx} className="bg-purple-600 text-white px-3 py-1 rounded-full text-xs font-semibold">
                                                {genre}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="flex gap-4 items-center flex-wrap">
                                    ...
                                    <span className="text-white text-lg font-semibold">
                                        {trendingMovies[currentMovieIndex].release_date?.split('-')[0] || 'N/A'}
                                    </span>
                                </div>
                                
                                {/* MOVIE DESCRIPTION */}
                                <p className="text-white text-lg max-w-2xl drop-shadow-lg line-clamp-4 leading-relaxed">
                                    {trendingMovies[currentMovieIndex].overview || 'No description available'}
                                </p>
                                
                                {/* ACTION BUTTONS */}
                                <div className="flex gap-4 pt-4">
                                    <button className="bg-linear-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white px-8 py-3 rounded-lg font-bold text-lg transition-all shadow-xl flex items-center gap-2">
                                        <Play size={20} fill="white" /> Watch Now
                                    </button>
                                    <button className="bg-white/20 hover:bg-white/30 text-white px-8 py-3 rounded-lg font-bold text-lg transition-all shadow-lg flex items-center gap-2 border border-white/30">
