...
const DEFAULT_CATEGORY = seriescategoryOption[0].value;
...
const Series = () => {
    ...
    const currentTitle = isGenreMode
        ? seriesGenreOptions.find((opt) => opt.value === genre)?.label
        : seriescategoryOption.find((opt) => opt.value === category)?.label;

    // Sync dropdown with route param (e.g. /series/popular)
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const endpoint = isGenreMode ? "discover/tv" : `tv${category}`;
                const params = isGenreMode
                    ? { with_genres: genre, sort_by: "popularity.desc", language: "en-US", page }
                    : { language: "en-US", page };
    ...
    }, [sentinel, isLoading, series.length]);
            ...
            <div className="relative z-50 mb-5 px-5 flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm md:text-base">
                        Discover trending TV series, explore genres, and find your next
                        binge-worthy show.
                    </p>
                </div>
                ...
                <div className="flex flex-wrap gap-4 items-center">
                    <CustomDropdown
                        ...
                        onChange={(value) => {
                            ...
                            navigate(categoryParam ? `/series/${categoryParam}` : "/series");
                        }}
                        options={seriescategoryOption}
                    ...
                    <CustomDropdown
                        ...
                        onChange={(value) => {
                            ...
                            if (!value) {
                                ...
                                navigate(defaultParam ? `/series/${defaultParam}` : "/series");
                            }
                        }}
                        options={seriesGenreOptions}
            ...
            <div className="relative z-10 max-w-7xl mx-auto mb-10">
                ...
                {series.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-20">
                        ...
                        <p className="text-gray-400 text-lg">Loading amazing series...</p>
                    </div>
                )}
            </div>
        </div>
